...
{
    private final int FALL_WAIT=50;
    ...
    public void collisions(char[][] frame)
    {
        ...
        // drawAreA area donde se dibuja el jugador
        ...
        /*
         * LLAVES
         * detecta si el jugador toca una llave
         */
		for( char[] caracteres: drawArea)
        {
			for( char caracter:caracteres)
        ...
        //colisones
        if(  this.getY() <= (frame.length-this.img.SIZE_Y+1) )    //evita out of ranges 
        {
            boolean PISO = false;
            ...
            {               
				if(caracter == '-')
                {
                    //mantiene al juegador en su posicio
					this.setY(this.getY()-1);  
					status = State.STATIC;
					break;
				}
    ...
    public void update(){
        this.captureInput();
        this.move();
        System.out.println("TECLA"+keyCode+" "+this.status);
    }
}